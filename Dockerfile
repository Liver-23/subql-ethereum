...
RUN printf '#!/bin/sh\n\necho "=== APPLYING ENTITY ID NORMALIZATION PATCH ==="\n\necho "Original file content around entityId:"\ngrep -n "entityId" /usr/local/lib/node_modules/@subql/node-ethereum/node_modules/@subql/node-core/dist/indexer/store/store.js || echo "No entityId found"\n\necho "Original file content around id assignment:"\ngrep -n "id.*=" /usr/local/lib/node_modules/@subql/node-ethereum/node_modules/@subql/node-core/dist/indexer/store/store.js | head -5\n\ncp -f /tmp/store.js.patched /usr/local/lib/node_modules/@subql/node-ethereum/node_modules/@subql/node-core/dist/indexer/store/store.js\n\necho "Entity ID normalization patch applied successfully"\necho "Original file backed up at /tmp/store.js.original"\n\necho "Patched file content around entityId:"\ngrep -n "entityId" /usr/local/lib/node_modules/@subql/node-ethereum/node_modules/@subql/node-core/dist/indexer/store/store.js || echo "No entityId found"\n\necho "Patched file content around id assignment:"\ngrep -n "id.*=" /usr/local/lib/node_modules/@subql/node-ethereum/node_modules/@subql/node-core/dist/indexer/store/store.js | head -5\n\nexec /usr/local/bin/subql-node-ethereum "$@"\n' > /tmp/subql-node-ethereum-wrapper && chmod +x /tmp/subql-node-ethereum-wrapper
