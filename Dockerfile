...
RUN printf '#!/bin/sh\n\necho "=== APPLYING ENTITY ID NORMALIZATION PATCH ==="\n\necho "Setting NODE_OPTIONS to preload monkey patch..."\nexport NODE_OPTIONS="--require /tmp/monkey-patch.js"\necho "Entity ID normalization patch will be applied before SubQuery starts"\nexec /usr/local/bin/subql-node-ethereum "$@"\n' > /tmp/subql-node-ethereum-wrapper && chmod +x /tmp/subql-node-ethereum-wrapper
